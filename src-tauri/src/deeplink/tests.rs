...
fn test_parse_codex_ticket_deeplink() {
    ...
    let url = format!(
        "tuziswitch://v1/import?resource=provider&app=codex&name=Codex&endpoint=https%3A%2F%2Fapi.tu-zi.com%2Fcoding&model=gpt-5.6-sol&enabled=true&ticket={ticket}&ticketUrl=https%3A%2F%2Fstore.tu-zi.com%2Fportal%2Fapi%2FcodexImport%2Fexchange"
    );
...
#[test]
fn test_rejects_invalid_or_non_codex_ticket_deeplink() {
    for url in [
        "tuziswitch://v1/import?resource=provider&app=codex&name=Codex&ticket=short",
        "tuziswitch://v1/import?resource=provider&app=claude&name=Claude&ticket=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "tuziswitch://v1/import?resource=provider&app=codex&name=Codex&ticketUrl=https%3A%2F%2Fstore.tu-zi.com%2Fportal%2Fapi%2FcodexImport%2Fexchange",
    ] {
        assert!(parse_deeplink_url(url).is_err(), "{url}");
    }
}
