...
const TICKET_CLIENT: &str = "tuzi";
const MAX_TICKET_RESPONSE_BYTES: usize = 64 * 1024;
...
async fn exchange_ticket(request: DeepLinkImportRequest) -> Result<DeepLinkImportRequest, String> {
    ...
    if request.resource != "provider"
        ...
        || ticket.len() != 64
        || !ticket.chars().all(|value| value.is_ascii_hexdigit())
    {
        return Err("配置票据参数无效".to_string());
    }
    ...
    if response
        .content_length()
        .is_some_and(|length| length > MAX_TICKET_RESPONSE_BYTES as u64)
    {
        return Err("配置服务返回内容过大".to_string());
    }
    let mut body_bytes = Vec::with_capacity(
        response
            .content_length()
            .unwrap_or_default()
            .min(MAX_TICKET_RESPONSE_BYTES as u64) as usize,
    );
    let mut stream = response.bytes_stream();
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|_| "读取配置服务响应失败".to_string())?;
        if body_bytes.len().saturating_add(chunk.len()) > MAX_TICKET_RESPONSE_BYTES {
            return Err("配置服务返回内容过大".to_string());
        }
        body_bytes.extend_from_slice(&chunk);
    }
    let body: serde_json::Value =
        serde_json::from_slice(&body_bytes).map_err(|_| "配置服务返回格式无效".to_string())?;
