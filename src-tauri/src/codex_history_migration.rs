...
fn collect_source_model_provider_ids(db: &Database) -> Result<BTreeSet<String>, AppError> {
    ...
    for provider in providers.values() {
        ...
        for provider_id in defined_custom_codex_model_provider_ids_from_config(config_text) {
...
fn collect_existing_state_db_provider_ids(
    ...
    let config_text = read_codex_config_text().unwrap_or_default();
    for db_path in codex_state_db_paths(codex_dir, &config_text) {
        ids.extend(existing_state_db_provider_ids(
            &db_path,
...
fn defined_custom_codex_model_provider_ids_from_config(config_text: &str) -> BTreeSet<String> {
    let Ok(doc) = config_text.parse::<DocumentMut>() else {
        return BTreeSet::new();
    };

    let mut ids = BTreeSet::new();
    insert_defined_custom_codex_model_provider_id(&mut ids, &doc, doc.get("model_provider"));
    if let Some(profiles) = doc.get("profiles").and_then(|item| item.as_table_like()) {
        for (_, profile_item) in profiles.iter() {
            if let Some(profile_table) = profile_item.as_table_like() {
                insert_defined_custom_codex_model_provider_id(
                    &mut ids,
                    &doc,
    ...
    if let Some(model_providers) = doc.get("model_providers").and_then(|item| item.as_table()) {
        for (provider_id, _) in model_providers.iter() {
            insert_defined_custom_codex_model_provider_id_str(&mut ids, &doc, provider_id);
        }
    }
    ids
}

fn insert_defined_custom_codex_model_provider_id(
    ...
    insert_defined_custom_codex_model_provider_id_str(ids, doc, provider_id);
}

fn insert_defined_custom_codex_model_provider_id_str(
    ids: &mut BTreeSet<String>,
    doc: &DocumentMut,
    provider_id: &str,
) {
    if provider_id.is_empty()
        || provider_id == CC_SWITCH_CODEX_MODEL_PROVIDER_ID
        || provider_id == OFFICIAL_OPENAI_CODEX_MODEL_PROVIDER_ID
...
fn codex_state_dbs_have_provider_ids(
    ...
    let config_text = read_codex_config_text().unwrap_or_default();
    for db_path in codex_state_db_paths(codex_dir, &config_text) {
        if codex_state_db_has_provider_ids(&db_path, source_provider_ids)? {
            return Ok(true);
        }
    }
    Ok(false)
...
fn backup_codex_state_db(
    ...
    copy_existing_file(db_path, &backup_path)
}
...
"#
    ...
    fn simulates_official_history_unify_migration_end_to_end() {
        ...
        assert_eq!(count_provider("tuziswitch"), 2);
