...
const EndpointSpeedTest: React.FC<EndpointSpeedTestProps> = ({
  ...
  const initialEndpointKeyRef = useRef("");
  ...
  const shouldPersistImmediately = variant === "inline" && isEditMode;

  useEffect(() => {
    const nextEntries = buildInitialEntries(initialEndpoints, value);
    const nextKey = nextEntries
      .map((entry) => entry.url)
      .sort()
      .join("\n");
    if (initialEndpointKeyRef.current === nextKey) return;
    initialEndpointKeyRef.current = nextKey;
    setEntries((current) => {
      const map = new Map<string, EndpointEntry>();
      ...
      current.forEach((entry) => {
        if (entry.isCustom && !map.has(entry.url)) {
          map.set(entry.url, entry);
        }
      });
      return Array.from(map.values());
    });
  }, [initialEndpoints, value]);
  ...
  const handleAddEndpoint = useCallback(async () => {
    ...
    if (shouldPersistImmediately && providerId) {
      setIsSaving(true);
      try {
  ...
  const handleRemoveEndpoint = useCallback(
    async (entry: EndpointEntry) => {
      ...
      if (shouldPersistImmediately && providerId && entry.isCustom) {
        setIsSaving(true);
        try {
  ...
  const content = (
    <div
      className={
        variant === "inline"
          ? "rounded-md border border-border-default bg-background p-4 flex flex-col gap-4"
          : "glass rounded-xl p-6 border border-white/10 flex flex-col gap-6"
      }
    >
      {/* 测速控制栏 */}
